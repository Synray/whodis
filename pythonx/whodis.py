    ...
    while True:
        ...
        if f == cur_dir:
            raise ValueError(
                    ...
                    + 'Run something like:\n' +
                    '  ninja -C out/Release -t compdb '
                    'cc cxx > compile_commands.json\n'
                    'in the source root.\n')
...
def _FindIndexForFile(cwd, file_directives, filename):
    for line in file_directives:
        ...
        if len(parts) < 2:
            continue
        index, dirname = parts
        candidate = os.path.normpath(os.path.join(cwd, dirname))
...
def _FindFunctionStart(asm_contents, start_index):
    ...
    while (not asm_contents[i].startswith('Lfunc_begin') and
            not asm_contents[i].startswith('.LFB') and
...
def _FindFunctionEnd(asm_contents, start_index):
    ...
    while (not asm_contents[i].startswith('Lfunc_end') and
            not asm_contents[i].startswith('.LFE') and
...
def Whodis():
    ...
    vim.command('autocmd CursorHold ' + buf.name + ' python whodis._UpdateSourceHover()')
    ...
    vim.command('silent! syn clear cStatement cppStatement cString cCppString')
    vim.command('silent! syn clear cConditional cRepeat cStorageClass cppStorageClass')
    vim.command('silent! syn clear cType cOperator cLabel cppType cppBoolean')
    vim.command('silent! syn clear cConstant cppConstant cBlock')
